...
impl EncodeTables {
    ...
    fn new(k: u8, m: u8) -> Self {
        ...
        let tables: Vec<Vec<GfMulTable>> = coeffs
            .iter()
            .map(|row| row.iter().map(|&c| GfMulTable::new(c)).collect())
            .collect();
...
static GF_EXP: [u8; 512] = [
    ...
    219, 171, 75, 150, 49, 98, 196, 149, 55, 110, 220, 165, 87, 174, 65, 130, 25, 50, 100, 200, 141,
    7, 14, 28, 56, 112, 224, 221, 167, 83, 166, 81, 162, 89, 178, 121, 242, 249, 239, 195, 155, 43,
    86, 172, 69, 138, 9, 18, 36, 72, 144, 61, 122, 244, 245, 247, 243, 251, 235, 203, 139, 11, 22,
...
impl ArmEncoder {
    ...
    pub fn new(k: u8, m: u8) -> Self {
        ...
        Self {
            level,
            encode_tables,
            fallback: CauchyEncoder::new(config),
            k,
            m,
        }
    }
        ...
        for coeff in 1..=255u8 {
            ...
            for i in 0..16u8 {
                ...
                assert_eq!(
                    ...
                    "coeff={}, lo_table[{}]: {} != {}",
                    coeff, i, table.lo[i as usize], gf_mul_portable(coeff, i)
                );
                ...
                assert_eq!(
                    ...
                    "coeff={}, hi_table[{}]: {} != {}",
                    coeff, i, table.hi[i as usize], gf_mul_portable(coeff, i.wrapping_mul(16))
                );
            }
            ...
            for b in 1..=255u8 {
                ...
                assert_eq!(
                    ...
                    "coeff={}, b={}: lo[{}]^hi[{}] = {} != {}",
                    coeff, b, lo_idx, hi_idx, reconstructed, gf_mul_portable(coeff, b)
                );
            }
        }
    }
