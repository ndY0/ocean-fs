...
//! `PoolRegistry` from a 4-pool config and assert all four roots are probed
...
fn pool_registry_builds_from_four_pool_config_and_probes_all_roots() {
    ...
    // All four roots were probed: created, and no probe litter remains.
    for root in &roots {
    ...
    // All four pools registered with config-order ids and correct roles.
