...
| f2 | [remote-fault-injectors](f2-remote-fault-injectors.md) | proposed | critical | f1 | SSH device yank/replug, per-volume disk fill, segment discovery + corruption, network latency on the internal interface, `RemoteCluster` POST-with-body + SSH exec, `FailureInjectionRecord` wiring |
