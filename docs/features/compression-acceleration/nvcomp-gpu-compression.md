...
- [ ] **Code:** `cargo build --features cuda` succeeds; `cargo build --all-targets` (no features) also succeeds
...
- [ ] **Lint:** `cargo clippy -- -D warnings` passes; every `unsafe` FFI block has `// SAFETY:` comment
- [ ] **Docs:** `#![deny(missing_docs)]` passes; `NvcompCompressor` docs document nvCOMP library requirements, codec options, and batch behavior
...
- [ ] **Integration:** `tests/nvcomp_roundtrip.rs` (requires GPU + nvCOMP): compress 64 MB segment batch with nvCOMP, decompress, verify bit-exact match; configure nvCOMP without GPU → verify fallback to zstd; batch size boundary (exactly 1, exactly nvcomp_batch_size)
- [ ] **Manual:** Example in `NvcompCompressor` docs compiles and runs with nvCOMP library installed
