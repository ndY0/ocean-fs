...
- [x] **ADR:** ADR-0006 constraints satisfied — trait-based pluggability (§3 for Compressor modeled on Encoder/Decoder), per-bucket only `compress_tier` (§5 Non-EC acceleration scope, §7 per-bucket tier selection), fallback chain with warnings (§2)
<!-- REVIEW: iteration-3: All constraints satisfied. §3: ✅ Compressor trait with Arc<dyn Compressor>. §5/§7: ✅ Amended by ADR-0007 — node ceiling + per-bucket opt-down implemented (cap_compression_tier in dispatcher.rs:360). §2: ✅ Fallback chain with tracing::warn! AND AtomicU64 metric counter (compression_fallback_count) incremented on each fallback event (dispatcher.rs:446,455). -->
