...
- [ ] **Invariant — one resolution, one owner:** the `sealing_data` field,
...
- [ ] **Read-after-write across every window:** parameterized test — GET
...
- [ ] **In-flight window closed by the seal transition:** after
...
- [ ] **No read gap under seal-queue backpressure:** with the seal channel
      at capacity, in-flight segments remain readable (structural
      improvement over ADR-0021's drop-on-full cleanup); the seal is
      retried, not skipped.
- [ ] **Perf 7.1:** the `ActiveSlot` serve holds only the slot lock for a
...
- [ ] **Integration:** server-crate round trip — PUT → GET immediately
