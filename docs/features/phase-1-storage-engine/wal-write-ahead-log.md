...
- [ ] **Tests:** Unit tests for append/replay round-trip, group commit batching, truncate-and-replay (only un-truncated entries replayed), concurrent append ordering
- [ ] **Coverage:** `cargo tarpaulin --fail-under 80` on `oceanfs-storage`
- [ ] **Lint:** `cargo clippy -- -D warnings` passes
- [ ] **Docs:** `#![deny(missing_docs)]` passes; `WalWriter` and `WalReader` have `# Examples`
- [ ] **ADR:** ADR-0001 segment packing is referenced
- [ ] **Perf:** Rules 3.1 (sequential-only), 3.4 (group commit), 3.5 (io_uring), 2.6 (bounded channels) verified
- [ ] **Integration:** `tests/wal_recovery.rs`: write entries, simulate crash (drop writer without truncate), open reader, verify all entries replayed; truncate partial, verify only remaining entries replayed
- [ ] **Manual:** `WalWriter` example in docs compiles and runs
