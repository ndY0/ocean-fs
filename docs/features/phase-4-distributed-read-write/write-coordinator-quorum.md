...
- [ ] **Docs:** `#![deny(missing_docs)]` passes; `WriteCoordinator::put` fully documented
- [ ] **ADR:** N/A (spec §7.1 covers quorum model)
- [ ] **Perf:** Rule 2.6 (bounded write queue), 4.5 (adaptive timeout: WAL write 100-500ms, metadata 10-50ms), 9.3 (HashKey pre-computed)
- [ ] **Integration:** `tests/write_quorum.rs`: 3-node cluster, PUT with W=2, verify data on 2 replicas, kill one node mid-write, verify quorum still met, verify data available on surviving nodes
